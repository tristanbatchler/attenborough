...
- After changing source SQL or schema, regenerate with `uv run sqlc generate --file src/attenborough/db/sqlc.yaml` (from `api/`) and inspect the generated diff, ignoring whitespace (`git diff -w`). Generated files are not ruff-formatted, so formatting-only churn is expected.
