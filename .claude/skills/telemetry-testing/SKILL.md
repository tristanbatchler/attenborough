...
3. From `api/`: `uv run python scripts/telemetry_probe.py verify --run <descriptive-tag>`. It also checks that each row's path and query are exactly what was sent (`TARGET`) and its body size (`BODY`). Expect `failures=0` and exit 0. Check the "Outcomes exercised" list actually contains 2xx, 303, 401 (returned and raised), 404, 405, 422 and 500 — the login decoys are random, so rerun if a branch was missed.
...
4b. For any change to the decoy app (`decoy/`), `api/src/attenborough/ingest/` or `telemetry.py`, also verify through the decoy app, which reports each request to the API's `/ingest/hits`. Build it (`npm run build` in `decoy/`) and start it in the background on port 8766, pointing at the server from step 2: `cd decoy && API_BASE_URL=http://127.0.0.1:8765 PORT=8766 exec node build > <scratchpad>/decoy.log 2>&1`. Then, from `api/`:
...
5. Scan the server log for unexpected errors: `grep -v "^DEBUG\|^INFO" <scratchpad>/server.log`. Expect none: every case is a handled response (the admin decoy's 500 is a raised `HTTPException`). `verify` has no unhandled-exception case, because nothing in the API deliberately crashes; don't reuse a real bug as one.
...
| `TARGET db≠sent` | The path or query was recorded as parsed, not as sent: the middleware must read `raw_path` and `query_string`, and the decoy app `event.platform.req.url`, never a normalised URL. |
| `BODY db≠sent` | The decoy app didn't report the body (or reported it cut short), or the API stored a body for its own routes, where it must be NULL. |
