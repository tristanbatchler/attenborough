...
Layout: `api/` (FastAPI backend, see `api/README.md`), `web/` (SvelteKit exhibit frontend, see `web/README.md`), `decoy/` (SvelteKit app serving the fake sites visitors see, reporting to the API; see `decoy/README.md`), `mise.toml` (tool versions and every check), `.claude/` (these rules and the skills).
...
- Decoys: `decoy/`, a second SvelteKit app with the same stack and checks as `web/`. It only presents pages: every request it serves is reported to the API's `/ingest/hits`, and submitted credentials go to `/ingest/logins`, which records them and decides the outcome. Decoys are `+server.ts` endpoints rendering Svelte components (`htmlPage`), never SvelteKit pages, and nothing they send may look like SvelteKit. Every hit is recorded exactly as sent: raw path and query, and the body (`telemetry_hits.query`, `body`, `body_size`). See `decoy/README.md` and `.claude/rules/decoy.md`. The API's own honeypot routers (`api/src/attenborough/honeypot/`) are being moved there.
...
- **After every change, run `mise run fix` then `mise run check` from the repo root; `check` must exit 0.** It is the gate for every part: `ruff check`, `ruff format --check` and `basedpyright` (API), `pytest`, then `svelte-check`, `eslint`, `prettier --check` and `vite build` (web, then decoy). It never skips a check and stops at the first failure. The tools pair up: ruff check ↔ eslint (linters), ruff format ↔ prettier (formatters), basedpyright ↔ svelte-check/TypeScript (type checkers). Each is also a task on its own (`mise tasks ls`). Report the actual output, never "passed" from memory.
...
- Generated code is excluded from every checker that allows it: ruff and basedpyright (`api/pyproject.toml`), ESLint and Prettier (`web/`, `decoy/`). Keep any new generated file excluded too. svelte-check can't exclude imported files, which is why `exactOptionalPropertyTypes` stays off.
