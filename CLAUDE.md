...
- Also run the magic-string scan after every change to the Python code (`mise run api-magic-strings`, judged and fixed with the `magic-strings` skill). It is not part of `check`, because accepted candidates remain. The user prefers enums and named constants to magic and repeated strings.
